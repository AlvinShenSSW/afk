...
On 2026-09-14 the owner adopted [release 1.2.0 with direction kept off](release-1.2.0-scope.md) and instructed continued investigation afterward. The [rollout guide](../../direction-rollout.md) records that choice, release scope and residual evidence. This completes C2's decision input; current reviews, checks and actual publication remain separate delivery steps. Missing #113 behavior and Epic outcomes remain OUTSTANDING.

The design and evidence attribution below preserve the original pre-decision contract. References to a PENDING choice or current historical report below describe that planning state, not a second live decision. No default, runtime, model or allowance changes accompany this decision.
