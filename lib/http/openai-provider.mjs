...
export function makeOpenAiProvider(cfg) {
  return {
    ...
    async complete({ system, user, model, maxTokens, env, fetchImpl, httpTimeoutMs = 120000 }) {
      ...
      if (!key) throw providerError('no_key', `${cfg.keyEnv} not set`);
      ...
      try {
        ...
        if (error?.name === 'AbortError') {
          throw providerError('timeout', `${cfg.name} request timed out after ${httpTimeoutMs}ms`);
        }
        const detail = transportDetail(error, key);
        throw providerError('transport', `${cfg.name} transport error${detail ? ` (${detail})` : ''}`);
      }
      ...
      try {
        if (!response.ok) {
          ...
          throw providerError(code, `${cfg.name} HTTP ${response.status}`);
        }
        ...
        try {
          ...
          throw providerError('bad_json', `${cfg.name}: response was not valid JSON`);
        }

        const choice = json?.choices?.[0];
        const completion = (cfg.extractText ? cfg.extractText(json) : choice?.message?.content) || '';
        if (!String(completion).trim()) {
          throw providerError('empty', `${cfg.name}: empty completion`);
        }
