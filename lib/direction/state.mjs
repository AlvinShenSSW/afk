...
function verifyReferences(operation, payload, proof) {
  for (const { reference, path } of references(payload)) {
    const opaque = operation === 'reserve' && equal(path, ['binding', 'packet'])
      || operation === 'terminal' && equal(path, ['terminal', 'result']);
    proof(reference, opaque ? 'opaque' : 'source');
  }
}
function inspect({ cwd, runId, issueId }) {
  ...
  const proof = (ref, validation = 'source') => {
    ...
    if (!evidenceCache.has(key)) {
      const bytes = load(location.run, ref.path, LIMITS.evidenceBytes, { sensitive: validation !== 'opaque' });
