...
export const DEFAULT_POLICY = Object.freeze({ mode: 'off', maxAuditAttempts: 4 });
...
export function validatePolicy(value) {
  ...
  oneOf(value.mode, MODES); integer(value.maxAuditAttempts); shape(value.sources, ['mode', 'maxAuditAttempts']);
  for (const field of ['mode', 'maxAuditAttempts']) {
    ...
    if (item.kind === 'built-in') {
      requireValue(item.source === null && value[field] === DEFAULT_POLICY[field], 'invalid_builtin_source');
...
export function foldRecords(records) {
  ...
  for (const record of records) {
    ...
    if (record.operation === 'initialize') {
      ...
      if (record.operation === 'baseline') { baselineTransition(p.baseline, baseline); baseline = p.baseline; }
        ...
        requireValue(history.priorAttempts.length + attempts.size < policy.maxAuditAttempts, 'attempts_exhausted');
  ...
  const count = history ? { knowledge: history.knowledge, charged, reserved,
    remaining: history.knowledge === 'known' ? Math.max(0, policy.maxAuditAttempts - charged) : null } : null;
  return { head, baseline, baselineDigest: baseline && contentDigest(baseline), policy, policyDigest: policy && contentDigest(policy),
    ...
    canReserve: Boolean(policy && policy.mode !== 'off' && count.knowledge === 'known' && count.remaining > 0) };
}
