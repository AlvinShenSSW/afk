...
export function loadPrepared({ cwd = process.cwd(), runId, issueId, auditId }, { historical = false } = {}) {
  ...
  // Old valid findings retain their exact request format without qualifying current dispatch.
  let matches = request === JSON.stringify(expected);
  if (!matches && historical) {
    expected.messages[1].content = requestContent(packet, false);
    matches = request === JSON.stringify(expected);
  }
