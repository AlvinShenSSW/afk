...
import { readFileSync, writeFileSync, mkdirSync, mkdtempSync, realpathSync, rmSync, symlinkSync, copyFileSync } from 'node:fs';
...
import { prepareAudit, checkAudit, observeTarget, validatePacket, validateModelResult,
  extractModelResult, validateDerivation, requestBody, profileFingerprint, validateQualification, RUNTIME_FILES, terminalRequest } from './audit.mjs';
import { contentDigest } from './schema.mjs';
import { recordExchange } from './transport.mjs';
...
test('own reservation may consume last slot; profile pending stays explicit', t => {
  const f = prepared(t);
  ...
  const check = checkAudit({ ...f, auditId: f.input.auditId, stage: 'pre-dispatch' });
  ...
  assert.equal(checkAudit({ ...f, auditId: f.input.auditId, stage: 'pre-dispatch' }).status, 'stale');
});
...
for (const phase of ['initial', 'signal', 'endpoint']) for (const outcome of ['ON-TRACK', 'CORRECT-COURSE', 'NEEDS-DECISION', 'COMPLETE']) {
  test(`${phase} ${outcome} cannot substitute for a qualified current endpoint COMPLETE`, async t => {
    ...
    const checked = checkAudit({ ...where, stage: 'endpoint', endpointId: packet.endpoint.id });
...
test('off preserves the old path, shadow remains shadow on unavailability, and full oversized input refuses', t => {
  ...
  appendDirectionRecord({ cwd: f.cwd, request: JSON.parse(readFileSync(join(prepared.directory, 'reserve-request.json'), 'utf8')) });
  const checked = checkAudit({ ...f, auditId: f.input.auditId, stage: 'pre-dispatch' }); assert.equal(checked.mode, 'shadow');
  ...
  const large = fixture(t); large.input.nextAction = 'Full supplied source context. '.repeat(1000);
  assert.throws(() => prepareAudit({ ...large, input: large.input }), /profile_input_limit/);
