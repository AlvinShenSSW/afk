...
const RULES = [
  ...
  {
    re: /(?<=(?:^([^\n]*)\n(?:[ \t]*\n)?)?)^([^\n]*?)\b[A-Fa-f0-9]{64,}\b[^\n]*$/gm,
    to: (match, previous, prefix) => {
      const label = /\b(?:sha-?256|digest|checksum|hash)\b/i;
      const labelled = label.test(prefix) || (/^[\s`*\-|>]*$/.test(prefix) && previous !== undefined && label.test(previous));
      return labelled ? match : match.replace(/\b[A-Fa-f0-9]{64,}\b/g, '[REDACTED]');
    },
  },
  {
