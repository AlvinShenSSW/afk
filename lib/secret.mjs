...
const RULES = [
  ...
  // A 64+ hex run labelled on its own line as a digest, checksum or hash is a documented content
  // identifier (authors pin baseline and policy digests in plans and ledgers); an unlabelled run stays redacted.
  // The label may sit on the same line before the run, or on the preceding line when a
  // formatter wrapped the value onto its own line (optionally across one blank line).
  {
    re: /(?<=(?:^([^\n]*)\n(?:[ \t]*\n)?)?)^([^\n]*?)\b[A-Fa-f0-9]{64,}\b[^\n]*$/gm,
    to: (match, previous) => {
      // Each run is judged by the text between it and the previous run on its line, so a labelled digest
      // never shelters an unrelated hex value later on the same line.
      const label = /\b(?:sha-?256|digest|checksum|hash)\b/i; let out = ''; let last = 0; let first = true;
      for (const run of match.matchAll(/\b[A-Fa-f0-9]{64,}\b/g)) {
        const before = match.slice(last, run.index);
        const labelled = label.test(before) || (first && /^[\s`*\-|>]*$/.test(before) && previous !== undefined && label.test(previous));
        out += before + (labelled ? run[0] : '[REDACTED]'); last = run.index + run[0].length; first = false;
      }
      return out + match.slice(last);
    },
  },
  {
    re: /(?<![A-Za-z0-9+/])[A-Za-z0-9+/]{40,}={0,2}(?![A-Za-z0-9+/])/g,
    to: (match, offset, text) => {
      if (/^[a-f0-9]+$/.test(match)) return match;
      // Base64 secrets of this length carry digits; a digitless run is prose such as
      // a slash-separated word list ("Integrity/concurrency/reliability/performance").
      if (!/[0-9]/.test(match) && !/[+=]/.test(match)) return match;
      // A path is exempt only with path context: a slash inside the run, no plus or padding, and the run
      // adjoining a dot, slash or tilde before it or a dotted extension after it. Slash count alone is not proof.
      if (match.includes('/') && !/[+=]/.test(match)) {
        const before = text[offset - 1] ?? ''; const after = text.slice(offset + match.length, offset + match.length + 2);
        if (/[./~]/.test(before) || /^\.[A-Za-z0-9]/.test(after)) return match;
      }
      return '[REDACTED]';
    },
  },
