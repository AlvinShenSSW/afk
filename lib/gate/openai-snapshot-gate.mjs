...
export async function runOpenAiSnapshotGate(config, {
  ...
  process.stderr.write(
    `[${config.slug}] POST model=${model} payload=${Buffer.byteLength(snapshot.payload, 'utf8')}B files=${snapshot.changedFiles.length}\n`,
  );
  ...
  emitReview(redactCredential(result.text, apiKey).text);
}
