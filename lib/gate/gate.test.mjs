...
test('a disabled gate still surfaces an unparseable target', () => {
  ...
  for (const [family, file] of [
    ...
    const earlyAt = source.indexOf('const early = parseTarget(userArgs)');
    const disabledAt = source.indexOf('isGateDisabled(');
    const disabledCallAt = source.indexOf('isGateDisabled(', source.indexOf('\n', disabledAt));
    assert.ok(earlyAt > 0, `${file} must check the target before it can skip`);
    assert.ok(
      earlyAt < Math.max(disabledAt, disabledCallAt),
