 ...
 */
// The same reader both target parsing and this guard use: one spelling rule.

export function guardFor(gateFamily, argv, { env = process.env } = {}) {
  const read = readOption(argv, '--implementer');
  const supplied = read.supplied;
  const value = read.value;
  // Two declarations cannot both be true, and resolving one silently is how a
  // wrapper appending its own overrules the caller's.
  if (read.duplicate) {
    return {
      run: false,
      reason: '--implementer was declared more than once; cannot prove this gate is independent, so it declines.',
    };
  }
...
export function stripImplementer(argv) {
  const equals = argv.findIndex((a) => typeof a === 'string' && a.startsWith('--implementer='));
  if (equals >= 0) return [...argv.slice(0, equals), ...argv.slice(equals + 1)];
  const i = argv.indexOf('--implementer');
  if (i < 0) return argv;
  // Consume the operand only when there is one: a following flag belongs to
  // the next option, and eating it would drop an unrelated argument.
  const next = argv[i + 1];
  const consumed = next !== undefined && !String(next).startsWith('--') ? 2 : 1;
  return [...argv.slice(0, i), ...argv.slice(i + consumed)];
}
