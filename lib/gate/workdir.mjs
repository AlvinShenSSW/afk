...
export function gateWorkDir(prefix, { tmp = tmpdir(), mkdtemp = mkdtempSync } = {}) {
  const root = tmp;
  try {
    return { path: mkdtemp(join(root, prefix)), error: null };
  } catch (err) {
    return {
      path: null,
      ...
      error: `cannot create this gate's working directory under ${root}: ${err.message}. `
