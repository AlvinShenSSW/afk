...
export function buildSnapshot({
  ...
  if (target.kind === 'uncommitted') {
    ...
    const prefix = gitTry(['rev-parse', '--show-prefix'], { cwd });
    if (!top.ok || !top.out.trim() || !prefix.ok) {
      return {
        error: `git could not resolve the worktree root${top.err || prefix.err ? `: ${top.err || prefix.err}` : ''}`,
        payload: '', changedFiles: [], notes: [], hasChanges: false,
      };
    }
    worktreeTop = top.out.trim();
    worktreePrefix = prefix.out.trim();
  }
  for (const entry of collected.entries) {
    const normalizedEntry = entry.status === '?'
      ? { ...entry, path: `${worktreePrefix}${entry.path}` }
      : entry;
    if (entryPaths(normalizedEntry).some((path) => isExcluded(path, extraExcludeGlobs))) {
      excludedCount++;
      excludedPaths.push(...entryPaths(normalizedEntry));
      continue;
    }
  ...
  const visibleFiles = renderedDiff.trim() && !truncated ? [...new Set(patchPaths)] : [];
