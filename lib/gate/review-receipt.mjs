...
const hashFields = new Set(['digest', 'startedDigest', 'revision', 'baseRevision', 'artifactDigest', 'head', 'baseTip']);
...
function metadata(value, credential = '', key = '') {
  if (typeof value === 'string') {
    requireCondition(!value.includes('\0'), 'binary receipt metadata');
    requireCondition(!credential || !value.includes(credential), 'receipt contains configured credential');
    if (!hashFields.has(key)) requireCondition(!redactCredential(value, credential).count, 'receipt contains sensitive metadata');
  } else if (Array.isArray(value)) value.forEach((item) => metadata(item, credential, key));
  else if (plain(value)) for (const [name, item] of Object.entries(value)) {
    metadata(name, credential); metadata(item, credential, name);
  }
...
export function beginReviewReceipt({ family, argv = [], cwd = process.cwd(), credential = '' } = {}) {
  ...
  const receipt = { enabled: true, argv: cleanArgs, request, family, path, error: null,
    capture(values) {
      ...
      if (before) metadata(before, credential);
  ...
  try {
    ...
    metadata(observed, credential); before = observed;
  }
