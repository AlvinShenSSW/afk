...
test('untracked paths below the root retain their repository prefix', () => {
  withRepo((dir) => {
    ...
    assert.deepEqual(snapshot.changedFiles, ['sub/same.txt']);
  });
});
...
test('an absolute design path ignores secret-like names above the repository root', () => {
  const dir = mkdtempSync(join(tmpdir(), 'credentials-work-'));
  try {
...
test('a code snapshot never exceeds its configured byte budget', () => {
  withRepo((dir) => {
    writeFileSync(join(dir, 'review.txt'), `${'€'.repeat(200)}\n`);
    const target = parseTarget(['--uncommitted'], { cwd: dir });
