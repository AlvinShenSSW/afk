...
test('a print group without --final-message-only is labelled, not silently taken', () => {
  const help = KIMI_CLI_HELP.replace('  --final-message-only        Print only the final message.\n', '');
  const { resolved, args } = argsFor(help);

  assert.equal(resolved.dialect, 'print-no-final-message');
  assert.equal(args.includes('--final-message-only'), false, 'never a flag this CLI does not document');
  assert.equal(resolved.transcriptRisk, true);
});
