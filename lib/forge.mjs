...
const ISSUE_COMMAND = {
  ...
  github: (id, { remoteUrl, repository } = {}) => {
    if (repository) return { bin: 'gh', args: ['issue', 'view', id, '--repo', repository] };
    const detected = detectForge(remoteUrl);
    if (detected && detected !== 'github') {
      return {
        unsupported: 'no GitHub repository — set `github-repository` under `## forge`',
      };
    }
    return { bin: 'gh', args: ['issue', 'view', id] };
  },
  ...
  'azure-devops': (id, { remoteUrl, organization } = {}) => {
    const org = organization || azureOrganization(remoteUrl);
    if (!org) {
      return {
        unsupported:
