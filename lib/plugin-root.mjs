...
import { isReleaseVersion } from './version.mjs';
...
export function resolvePluginRootUpdate(configured, resolved) {
  ...
  // Only a known, removed host cache is replaceable across installation styles.
  const knownAfkCache = /\/plugins\/cache\/afk\/afk-skills$/.test(currentCache?.base || '');
  let missing = false;
  try { statSync(current); } catch (error) { missing = error.code === 'ENOENT'; }
  if (knownAfkCache && missing && !targetCache) {
    try {
      const manifest = JSON.parse(readFileSync(join(target, '.claude-plugin', 'marketplace.json'), 'utf8'));
      if (manifest.name === 'afk' && manifest.plugins?.some((plugin) => plugin.name === 'afk-skills')
        && statSync(join(target, 'skills', 'afk', 'SKILL.md')).isFile()) {
        return { action: 'refresh', root: target, reason: 'the recorded afk cache is missing and the resolved afk install is live' };
      }
    } catch { /* An unverifiable replacement cannot displace the configured root. */ }
  }
