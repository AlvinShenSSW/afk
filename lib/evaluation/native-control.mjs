...
export function observeNativeControl({trial,plan,support,catalog,bodies,exchanges,effects=[],contextDelivery=[]}){
  ...
  try{
    ...
    for(const exchange of exchanges){
      ...
      requireControl(request?.status==='observed'&&response?.status==='observed','incomplete native exchange');
      ...
      for(const output of request.outputs){
        ...
        requireControl(call&&retained&&call.shape===callShape(retained)&&call.ordinal<ordinal,'unbound native output parent');seenOutputs.set(outputKey,output.digest);
      ...
      for(const call of response.response.output.filter(row=>['custom_tool_call','function_call'].includes(row.type))){
        const callKey=key(invocationId,sessionId,call.call_id);requireControl(!calls.has(callKey),'reused native call identity');
        calls.set(callKey,{shape:callShape(call),ordinal,invocationId,sessionId,callId:call.call_id});
      }
      for(const item of response.response.output.filter(row=>row.type==='reasoning'))assistantReasoning.add(key(invocationId,sessionId,digestBytes(JSON.stringify(item))));
    }
