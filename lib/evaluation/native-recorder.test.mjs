...
async function setup(t,options={}) {
  ...
  const reservations=[];
  const recorder=await createNativeRecorder({directory:join(root,'capture'),authMode:'chatgpt',credential:{bearer:upstreamBearer,accountId:'synthetic-account'},maxRequests:3,maxBytes:100000,
  ...
  t.after(()=>{localTokens.delete(recorder.url);return recorder.stop();});
...
test('profile denial retains response and usage but releases no executable SSE',async t=>{
  ...
  const calls=upstream(t,{body:denied});const {root,recorder}=await setup(t,{declarationDigest});
