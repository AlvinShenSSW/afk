...
function observedOptions(observer) {
  ...
  requireEvaluation(url.protocol==='http:'&&url.hostname==='127.0.0.1'&&url.port&&!url.username&&!url.password&&!url.search&&!url.hash&&['/responses','/backend-api/codex/responses'].includes(url.pathname),'invalid observed collector URL');
