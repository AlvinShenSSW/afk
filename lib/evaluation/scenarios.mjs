...
export function scoreControl({trial,observation={}}) {
  ...
  const loading=required.length&&required.every(r=>reads.some(x=>x.path===r.path&&x.digest===r.digest&&x.complete===true&&x.success===true&&prior(x)))?'pass':'unobserved';
  return {version:DIRECTION_VERSION,caseId:trial.caseId,selection:!native?'unqualified':observation.selectedSkill===trial.skill?'pass':'fail',loading,
    behavior:observation.behaviorEvidence===true?(observation.behaviorPass===true?'pass':'fail'):'unobserved',
