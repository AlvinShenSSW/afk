...
export function checkNativeRelease(decoded) {
  ...
  try {
    ...
    const items=decoded.response.output,ids=new Map(),calls=new Set(),stages=new Map();
    ...
    function supported(item) {
      ...
      if(item.type==='custom_tool_call') requireWire(item.namespace==='functions'&&item.name==='exec','direct custom call denied');
      else if(item.type==='function_call') requireWire(item.namespace==='functions'&&item.name==='wait'||item.namespace==='clock'&&item.name==='sleep','direct function call denied');
