...
import { checkNativeRelease, decodeNativeRequest, decodeNativeResponse, nativeDeclarationDigest } from './native-wire.mjs';
...
import { observedHostEnvironment } from './host.mjs';
...
export async function openObservedCollector({directory,id,workspace,support,model,catalog,manifest,codex,deadline,resume,priorId,env=process.env}){
  ...
  const session=prepareObservedSession(sessionOptions),root=join(directory,'artifacts',id),collectorRoot=join(root,'collector');
  ...
  return {session,observer:{url:recorder.url,readRoots:[session.protectedRoot,join(session.stateRoot,'skills/.system')]},
    environment:observedHostEnvironment({stateRoot:session.stateRoot,tempRoot:root,localToken:recorder.localToken}),finish};
}
...
export function readObservedInvocation({directory,id,workspace,support,model,catalog,manifest,codex,resume,priorId}){
  ...
  try{
    ...
    requireObserved(configuration.version===1&&configuration.authMode===observer.authMode&&configuration.maxRequests===observer.maxRequestsPerInvocation
    ...
    const physical=observedPhysicalUsage({directory,observer,executionHandoffDigest:manifest.executionHandoffDigest}),parent=physical.parents[id];
    ...
    requireObserved(names.length===stopped.requests&&names.length<=observer.maxRequestsPerInvocation,'collector request census');let complete=names.length>0;
    for(const [index,name]of names.entries()){
      ...
      requireObserved(terminal.version===1&&terminal.ordinal===ordinal&&typeof terminal.forwarded==='boolean'&&count(terminal.finishedAt)&&terminal.finishedAt<=stopped.stoppedAt,'collector terminal missing');
      ...
      const response=decodeNativeResponse(responseBytes,{maxBytes:observer.maxBytes,requestedModel:model,contentType:'text/event-stream'});
      const release=checkNativeRelease(response);
      requireObserved(response.status==='observed'&&equal(response,terminal.observation)&&equal(release,terminal.release),'observed response source changed');
      ...
      summary.exchanges.push({ordinal,forwarded:true,request:{path:`${prefix}-request.json`,digest:digestBytes(bytes)},response:{path:`${prefix}-response.body`,digest:digestBytes(responseBytes)},
        catalog:{path:`${prefix}-catalog.json`,digest:digestBytes(catalogBytes)},responseId:response.response.id,released:release.allowed&&terminal.reason===null});
