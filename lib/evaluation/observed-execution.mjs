...
export function readObservedInvocation({directory,id,workspace,support,model,catalog,manifest,codex,resume,priorId}){
  ...
  try{
    ...
    for(const [index,name]of names.entries()){
      ...
      const response=decodeNativeResponse(responseBytes,{maxBytes:observer.maxBytes,requestedModel:model,responseMetadata:terminal.responseMetadata});
...
function exchangeUsage(directory,record,observer){
  ...
  const observation=decodeNativeResponse(response,{maxBytes:observer.maxBytes,responseMetadata:terminal.responseMetadata,requestedModel:record.model});
