...
import { decodeNativeRequest, decodeNativeResponse, wireBound, nativeDeclarationDigest, checkNativeRelease } from './native-wire.mjs';
...
const FORWARD_HEADERS=['session_id','conversation_id','x-codex-turn-state','x-codex-turn-metadata','openai-beta'];
...
export async function createNativeRecorder(options) {
  ...
  async function handle(req,res) {
    ...
    const context={id,input:Buffer.alloc(0),output:Buffer.alloc(0),inputCount:0,outputCount:0,inputComplete:false,outputComplete:false,
      ...
      reservation:null,observation:null,release:null,status:null,reason:null,request:null,cancel:abort};
    active=context;
    ...
    try {
      ...
      responseBody=await Promise.race([cancelled,(async()=>{
        ...
        if(declarationDigest!==undefined){
          context.release=checkNativeRelease(context.observation);
