...
import { decodeNativeRequest, decodeNativeResponse, wireBound } from './native-wire.mjs';

const ROUTES=Object.freeze({
  chatgpt:'https://chatgpt.com/backend-api/codex/responses',
  'api-key':'https://api.openai.com/v1/responses',
});
const OPTIONS=['directory','authMode','maxRequests','maxBytes','concurrency','deadline','requestTimeoutMs','reserve'];
const FORWARD_HEADERS=['authorization','chatgpt-account-id','openai-organization','openai-project',
  'session_id','conversation_id','x-codex-turn-state','x-codex-turn-metadata','openai-beta'];
...
export async function createNativeRecorder(options) {
  ...
  const {authMode,maxRequests,maxBytes,concurrency,deadline,requestTimeoutMs,reserve}=options;
  ...
  if(!Object.hasOwn(ROUTES,authMode)) throw new Error('unsupported auth mode');
  const directory=join(realpathSync(dirname(options.directory)),basename(options.directory));
  mkdirSync(directory,{mode:0o700});
  const put=(name,value)=>writeFileSync(join(directory,name),Buffer.isBuffer(value)?value:canonicalBytes(value),{flag:'wx',mode:0o600});
  put('configuration.json',{version:1,authMode,upstream:ROUTES[authMode],maxRequests,maxBytes,concurrency,deadline,requestTimeoutMs,
    claim:'Wire evidence only; no host qualification or campaign authority is conferred.'});
  ...
  async function handle(req,res) {
    ...
    const context={id,input:Buffer.alloc(0),output:Buffer.alloc(0),inputCount:0,outputCount:0,inputComplete:false,outputComplete:false,
      ...
      reservation:null,observation:null,status:null,reason:null,request:null,cancel:abort};
    active=context;
    ...
    try {
      ...
      responseBody=await Promise.race([cancelled,(async()=>{
        const expected=new URL(ROUTES[authMode]).pathname;
        ...
        if(bytes.includes(Buffer.from(token))) throw failure('credential-in-body');
        ...
        const incoming=await new Promise((resolve,reject)=>{
          const upstream=https.request(ROUTES[authMode],{method:'POST',headers},resolve);context.upstream=upstream;
        ...
        if(responseBytes.includes(Buffer.from(token))) throw failure('credential-in-body');
        ...
        context.observation=decodeNativeResponse(responseBytes,{maxBytes,contentType:incoming.headers['content-type'],requestedModel:decoded.model});
      ...
      try {
        for(const [field,name] of [['input','request'],['output','response']]) {
          if(context[field].length&&!context[`${field}Published`]&&context.credential&&!context[field].includes(Buffer.from(context.credential))) {
            put(`${id}-${name}.partial`,context[field]);
          }
        }
        put(`${id}-terminal.json`,{version:1,ordinal:Number(id),forwarded:context.forwarded,status:context.status,reason:context.reason,
          ...
          observation:context.observation,finishedAt:Date.now()});
      } catch {latch('terminal-publication-failed');context.reason??='terminal-publication-failed';}
      active=null;
    }
    if(context.reason) reply(res,context.forwarded||context.reservation||!['unsupported-request','authentication-missing','input-byte-limit','request-uninterpretable','credential-in-body'].includes(context.reason)?502:400,JSON.stringify({error:context.reason}));
  ...
  deadlineTimer=setTimeout(()=>{latch('collector-deadline');void stop();},deadline-Date.now());
  const path=new URL(ROUTES[authMode]).pathname;
  return {url:`http://127.0.0.1:${server.address().port}${path}`,directory,stop};
}
