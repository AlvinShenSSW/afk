...
import { FORGES, azureOrganization, detectForge, issueCommand, resolveForge } from './forge.mjs';
...
test('an unrecognised host is left to gh, which refuses what it does not know', () => {
  // A self-hosted install is the common case behind an unrecognised host, and
  // gh declines a host it was never authenticated against — verified: it exits
  // non-zero with "none of the git remotes ... point to a known GitHub host".
  const cmd = issueCommand('github', 42, { remoteUrl: 'https://git.example.com/o/r.git' });
  assert.deepEqual(cmd, { bin: 'gh', args: ['issue', 'view', '42'] });
});
